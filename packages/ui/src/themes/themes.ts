import { generateAllThemePresets } from '@/lib/color-preset-generator'
import { themeColorsToCssVariables } from '@/lib/theme-to-css'
import { ThemeBase, ThemePreset } from '@/types'
import { BaseColorPreset, baseColors } from './base-colors'
...
interface _THEMETYPE {
  ...
  fontFamily?: Record<string, { name: string; type: string }>
  radius?: number
}
...
// const generatedPresets = generateAllThemePresets()
...
export const THEMEPRESETS: (ThemeBase & { id: ThemePreset; name: string })[] =
  _THEMES
    .map((theme) => ({
      name: theme.name,
      id: theme.id as ThemePreset,
      colors: {
        light: theme.colors,
        dark: theme.colorsDark
      },
      radius: theme.radius ?? 0.5
    })) //.concat(generatedPresets)
